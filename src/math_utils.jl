...
"""
    kahan_add(sum, comp, x) -> (new_sum, new_comp)

One-step compensated summation update. Use in long accumulation loops to
reduce cancellation error when positive and negative contributions nearly
balance.
"""
@inline function kahan_add(sum::Float64, comp::Float64, x::Float64)
    y = x - comp
    t = sum + y
    return t, (t - sum) - y
end
