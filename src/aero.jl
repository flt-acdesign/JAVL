...
function compute_forces!(sol::AVLSolution, vl::VortexLattice, config::AVLConfig,
    ...
    cdv_tot = 0.0
    c_cfx_tot = 0.0; c_cfy_tot = 0.0; c_cfz_tot = 0.0
    c_cmx_tot = 0.0; c_cmy_tot = 0.0; c_cmz_tot = 0.0
    c_cdv_tot = 0.0
    ...
    for js in 1:nstrip
        ...
        s_cnc = 0.0
        c_s_cfx = 0.0; c_s_cfy = 0.0; c_s_cfz = 0.0
        c_s_cmx = 0.0; c_s_cmy = 0.0; c_s_cmz = 0.0
        c_s_cnc = 0.0
        ...
        for k in 1:strip.nelem
            ...
            s_cfx, c_s_cfx = kahan_add(s_cfx, c_s_cfx, fx)
            s_cfy, c_s_cfy = kahan_add(s_cfy, c_s_cfy, fy)
            s_cfz, c_s_cfz = kahan_add(s_cfz, c_s_cfz, fz)

            # moment about reference point
            dx = elem.rv[1] - xyzref[1]
            dy = elem.rv[2] - xyzref[2]
            dz = elem.rv[3] - xyzref[3]
            s_cmx, c_s_cmx = kahan_add(s_cmx, c_s_cmx, (dy*fz - dz*fy) / bref)
            s_cmy, c_s_cmy = kahan_add(s_cmy, c_s_cmy, (dz*fx - dx*fz) / cref)
            s_cmz, c_s_cmz = kahan_add(s_cmz, c_s_cmz, (dx*fy - dy*fx) / bref)
            ...
            # spanloading c*Cn
            s_cnc, c_s_cnc = kahan_add(s_cnc, c_s_cnc,
                                       2.0 * gam_i * sqrt(gx^2 + gy^2 + gz^2) / sref)
        ...
        if strip.has_cdcl && length(strip.cdcl) >= 6
            ...
            cdv_tot, c_cdv_tot = kahan_add(cdv_tot, c_cdv_tot, cdv_force)
        end

        # accumulate
        cfx_tot, c_cfx_tot = kahan_add(cfx_tot, c_cfx_tot, s_cfx)
        cfy_tot, c_cfy_tot = kahan_add(cfy_tot, c_cfy_tot, s_cfy)
        cfz_tot, c_cfz_tot = kahan_add(cfz_tot, c_cfz_tot, s_cfz)
        cmx_tot, c_cmx_tot = kahan_add(cmx_tot, c_cmx_tot, s_cmx)
        cmy_tot, c_cmy_tot = kahan_add(cmy_tot, c_cmy_tot, s_cmy)
        cmz_tot, c_cmz_tot = kahan_add(cmz_tot, c_cmz_tot, s_cmz)
        strip_cnc[js] = s_cnc
