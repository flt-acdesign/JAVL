...
function trefftz_drag!(sol::AVLSolution, vl::VortexLattice, config::AVLConfig)
    ...
    for js in 1:nstrip
        ...
        for k in 1:strip.nelem
            ie = strip.ifirst + k - 1
            ie > nv && continue
            gams[js], c_gams[js] = kahan_add(gams[js], c_gams[js], sol.gam[ie])
        end
    end
    ...
    for jc in 1:nstrip  # receiving strip
        ...
        for jv in 1:nstrip  # sending strip
            ...
            g = gams[jv]
            if rsq1 > 1e-30
                vy[jc], c_vy[jc] = kahan_add(vy[jc], c_vy[jc], hpi * g *  dz1 / rsq1)
                vz[jc], c_vz[jc] = kahan_add(vz[jc], c_vz[jc], hpi * g * (-dy1) / rsq1)
            end
            if rsq2 > 1e-30
                vy[jc], c_vy[jc] = kahan_add(vy[jc], c_vy[jc], -hpi * g *  dz2 / rsq2)
                vz[jc], c_vz[jc] = kahan_add(vz[jc], c_vz[jc], -hpi * g * (-dy2) / rsq2)
            end
            ...
            if izsym != 0
                ...
                rsq2i = dy2^2 + dz2i^2
                if rsq1i > 1e-30
                    vy[jc], c_vy[jc] = kahan_add(vy[jc], c_vy[jc], -hpi * g * fz *  dz1i / rsq1i)
                    vz[jc], c_vz[jc] = kahan_add(vz[jc], c_vz[jc], -hpi * g * fz * (-dy1) / rsq1i)
                end
                if rsq2i > 1e-30
                    vy[jc], c_vy[jc] = kahan_add(vy[jc], c_vy[jc], hpi * g * fz *  dz2i / rsq2i)
                    vz[jc], c_vz[jc] = kahan_add(vz[jc], c_vz[jc], hpi * g * fz * (-dy2) / rsq2i)
                end
            end
            ...
            if iysym != 0
                ...
                rsq2i = dy2i^2 + dz2^2
                if rsq1i > 1e-30
                    vy[jc], c_vy[jc] = kahan_add(vy[jc], c_vy[jc], -hpi * g * fy *  dz1 / rsq1i)
                    vz[jc], c_vz[jc] = kahan_add(vz[jc], c_vz[jc], -hpi * g * fy * (-dy1i) / rsq1i)
                end
                if rsq2i > 1e-30
                    vy[jc], c_vy[jc] = kahan_add(vy[jc], c_vy[jc], hpi * g * fy *  dz2 / rsq2i)
                    vz[jc], c_vz[jc] = kahan_add(vz[jc], c_vz[jc], hpi * g * fy * (-dy2i) / rsq2i)
                end
            end
            ...
            if iysym != 0 && izsym != 0
                ...
                rsq2i = dy2i^2 + dz2i^2
                if rsq1i > 1e-30
                    vy[jc], c_vy[jc] = kahan_add(vy[jc], c_vy[jc], hpi * g * fyz *  dz1i / rsq1i)
                    vz[jc], c_vz[jc] = kahan_add(vz[jc], c_vz[jc], hpi * g * fyz * (-dy1i) / rsq1i)
                end
                if rsq2i > 1e-30
                    vy[jc], c_vy[jc] = kahan_add(vy[jc], c_vy[jc], -hpi * g * fyz *  dz2i / rsq2i)
                    vz[jc], c_vz[jc] = kahan_add(vz[jc], c_vz[jc], -hpi * g * fyz * (-dy2i) / rsq2i)
                end
            end
        end
    end
    ...
    for jc in 1:nstrip
        ...
        # far-field lift
        clff, c_clff = kahan_add(clff, c_clff, 2.0 * gams[jc] * dyt / sref)

        # far-field side force (NOTE: negative sign, matching AVL)
        cyff, c_cyff = kahan_add(cyff, c_cyff, -2.0 * gams[jc] * dzt / sref)
        ...
        cdff, c_cdff = kahan_add(cdff, c_cdff,
                                 gams[jc] * (dzt*vy[jc] - dyt*vz[jc]) / sref)
    end
